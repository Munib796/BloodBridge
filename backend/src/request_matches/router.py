...
@router.post("/{request_id}/accept", response_model=dtos.RequestMatchDetailOut, status_code=201)
...
@router.get("/mine", response_model=list[dtos.RequestMatchDetailOut])
...
@router.patch("/{match_id}/eta", response_model=dtos.RequestMatchDetailOut)
def update_eta(
    match_id: str,
...
@router.patch("/{match_id}/cancel", response_model=dtos.RequestMatchDetailOut)
def cancel_match(
    match_id: str,
...
@router.patch("/{match_id}/complete", response_model=dtos.RequestMatchDetailOut)
def complete_match(
    match_id: str,
    actor: Identity = Depends(get_current_acceptor),
    db: Session = Depends(get_db),
