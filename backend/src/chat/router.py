...
from src.chat.controller import get_current_chat_participant
...
@router.get("/threads", response_model=list[dtos.ChatThreadSummaryOut])
def list_my_threads(
    limit: Annotated[int, Query(ge=1, le=100)] = 50,
    offset: Annotated[int, Query(ge=0)] = 0,
    participant: Identity = Depends(get_current_chat_participant),
    db: Session = Depends(get_db),
):
    """This account's conversations, most recently active first.

    Declared before the /{match_id}/... routes for readability only — the
    path shapes don't actually collide.
    """
    return controller.list_my_threads(participant, db, limit, offset)
