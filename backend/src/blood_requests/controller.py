...
def assert_can_view_commitments(blood_request: BloodRequest, identity: Identity) -> None:
    """Who may see *who* signed up for a request, as opposed to the request
    itself.

    Deliberately narrower than assert_can_view_request: a donor can always see
    their own commitment through /request-matches/mine, but is never shown who
    else responded. That leaves the poster, the hospital verifying the request,
    and an admin.

    Same 404-rather-than-403 reasoning as assert_can_view_request — a 403 here
    would confirm that the request ID exists.
    """
    if identity.is_admin:
        return
    ...
    raise HTTPException(
        status_code=status.HTTP_404_NOT_FOUND,
        detail="Request not found, or its commitments are not visible to you",
    )
...
def assert_acceptor_eligible(blood_request: BloodRequest, identity: Identity, db: Session) -> None:
    ...
    For a donor this also enforces the Available/Unavailable toggle. Note that
    only *new* acceptances are refused: a donor who goes unavailable keeps any
    commitment they already hold, and can still cancel or complete it.
    """
    ...
    # An organization is dual-capable — it can post requests *and* accept them —
    # so nothing else stops it from accepting its own. Without this guard it
    # would instantly inflate units_secured on a request it posted itself,
    # flipping it to partially/fully matched and pulling it out of the nearby
    # feed, with no donor anywhere in the picture. Requestors can't reach this
    # path at all: get_current_acceptor admits only donors and organizations.
    if identity.role == "organization" and blood_request.organization_id == entity.id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="You cannot accept a request your own organization posted",
        )

    if identity.role == "donor":
        # Checked before compatibility and distance: an unavailable donor has
        # one obvious fix, and telling them their blood type is wrong instead
        # would send them looking in the wrong place.
        if not entity.is_available:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    "You are marked unavailable — switch yourself back to available "
                    "to accept requests"
                ),
            )
...
def list_my_requests(
    ...
    Unlike GET /blood-requests/{request_id} there is no visibility rule to
    apply: the query is scoped to the caller's own rows, so this endpoint can't
    be used to reach another poster's request.

    `statuses` is the history screen's filter — repeated `?status=` params. An
    omitted filter means "everything", including drafts and rejected requests,
    because that is what a history view wants.
    """
    query = db.query(BloodRequest)
    if identity.role == "requestor":
        query = query.filter(BloodRequest.requestor_id == identity.entity.id)
    elif identity.role == "organization":
        query = query.filter(BloodRequest.organization_id == identity.entity.id)
    else:
        # get_current_poster only admits these two roles; this is a guard
        # against the function being reused somewhere less careful.
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only a poster can list their own requests"
        )

    if statuses:
        query = query.filter(BloodRequest.status.in_(statuses))

    return (
        query
        # id is the tiebreaker: created_at is a transaction timestamp, so two
        # requests posted in the same transaction would otherwise be ordered
        # arbitrarily and OFFSET pagination could repeat or skip one.
...
def auto_widen_stale_requests(db: Session) -> int:
    """Widen the radius of open requests that have gone stale without a match."""
    now = datetime.now(timezone.utc)
    candidates = (
        db.query(BloodRequest)
        .filter(BloodRequest.status.in_(OPEN_STATUSES))
