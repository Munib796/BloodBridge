...
def find_donors_to_notify(blood_request: BloodRequest, db: Session) -> list[Donor]:
    ...
    * not a donor who already holds an accepted commitment on this request —
    ...
    Plus the conditions the donor-facing feed got for free from being
    authenticated: the account is active, the donor has not switched themselves
    to unavailable, and it has a location to match on. Donors without a
    device_token are skipped — there is nowhere to send to.
    """
