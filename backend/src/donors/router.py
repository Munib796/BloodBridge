...
@router.patch("/me/availability", response_model=dtos.DonorOut)
def update_availability(
    data: dtos.DonorUpdateAvailability,
    donor: Donor = Depends(get_current_donor),
    db: Session = Depends(get_db),
):
    """Set the donor's Available/Unavailable state. An unavailable donor is not
    notified about new requests and cannot accept one — send
    `{"is_available": false}` to step back without clearing the location."""
    return controller.update_availability(donor, data, db)
...
@router.delete("/me/profile-pic", response_model=dtos.DonorOut)
def remove_profile_pic(
    donor: Donor = Depends(get_current_donor),
    db: Session = Depends(get_db),
):
    return controller.remove_profile_pic(donor, db)
...
def delete_account(
    ...
    controller.delete_account(donor, db)
