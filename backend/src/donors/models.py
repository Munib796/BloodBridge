...
class Donor(Base):
    ...
    # An explicit "not right now" on top of having a location. An unavailable
    # donor is skipped by notification targeting (notifications.find_donors_to_
    # notify) and is refused when accepting a request (blood_requests.controller
    # .assert_acceptor_eligible) — the toggle means what it says. Defaults to
    # available so a donor who never touches it matches exactly as before this
    # flag existed; a donor with no location is unreachable either way.
