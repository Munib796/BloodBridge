...
def remove_profile_pic(requestor: Requestor, db: Session) -> Requestor:
...
def delete_account(requestor: Requestor, db: Session) -> None:
    """Deactivate the requestor while preserving historical requests."""
    requestor.is_active = False
    requestor.full_name = "Deleted requestor"
    requestor.email = f"deleted-{requestor.id}@deleted.bloodbridge.local"
